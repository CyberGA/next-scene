import { MovieTeaserData } from "@/definitions/movie.interface";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await TMDB_Request.get(
      `${TMDB_Endpoints.fetchMovie}/${id}/videos?language=en-US`
    );
    const data = (response.data.results as Array<MovieTeaserData>).filter(
      (data) => data.type === "Trailer" || data.type === "Teaser"
    );
